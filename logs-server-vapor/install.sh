...
PLIST_NAME=com.eduardocamaratta.lockstatelogsserver.plist;
PLIST_FULL_PATH=plists/$PLIST_NAME;
PLIST_FULL_TARGET_PATH=$HOME/Library/LaunchAgents/$PLIST_NAME;
sed "s@\$BINARY_ABSOLUTE_PATH@$BIN_TARGET_FULL_PATH@" $PLIST_FULL_PATH > $PLIST_FULL_TARGET_PATH;

# Start the daemon
launchctl load $PLIST_FULL_TARGET_PATH;